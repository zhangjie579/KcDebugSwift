...
BOOL zp_isObjcObject(const void *inPtr, CFMutableSetRef registeredClasses) {
    ...
    */
    if ((isa & ~MLeaks_ISA_MASK) == 0) {
        ptrClass = (Class)isa;
    } else {
        // jerrychu: 即使是non-pointer isa, isa & isa_magic_mask == isa_magic_value 条件也不成立，先取消判断。
        // isa & isa_magic_mask != isa_magic_value
        ptrClass = (Class)(isa & MLeaks_ISA_MASK);
//        if ((isa & MLeaks_ISA_MAGIC_MASK) == MLeaks_ISA_MAGIC_VALUE) {
//            ptrClass = (Class)(isa & MLeaks_ISA_MASK);
//        } else {
//            ptrClass = (Class)isa;
//        }
    ...
    if (CFSetContainsValue(registeredClasses, ptrClass)) {
        isKnownClass = true;
    }
...
//    NSString *clsName = NSStringFromClass(ptrClass);
//    if ([clsName.lowercaseString containsString:@"sawa"]) {
//        NSLog(@"ff --- %@", clsName);
//    }
