...
NSMutableDictionary<NSValue *, id> *strongAssociations(id object) {
    std::lock_guard<std::mutex> l(*_associationMutex);
    if (_associationMap->size() == 0 ){
        return nil;
    }
    ...
    NSMutableDictionary<NSValue *, id> *dict = [[NSMutableDictionary alloc] init];
    for (auto &key: *refs) {
        id value = objc_getAssociatedObject(object, key);
        if (value) {
...
#if _INTERNAL_RCD_ENABLED
  return FB::AssociationManager::strongAssociations(object);
#else
  return nil;
#endif //_INTERNAL_RCD_ENABLED
}
